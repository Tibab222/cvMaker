...
export default function EducationPicker() {
  ...
  const { toggleEducation, selection } = useCVSelection();

  return (
    <div className="space-y-3 p-1">
      <AnimatePresence>
        {education.map((edu, index) => {
          const isSelected = selection.selectedEducationIds.includes(edu.id);
              ...
              <div className="pr-6">
                ...
                <div className="flex items-center gap-3 mt-3">
                  <div className="flex items-center gap-1 text-[11px] font-medium text-muted-foreground uppercase tracking-wider">
                    <Calendar size={12} />
                    <span>
                      {new Date(edu.startDate).getFullYear()} - {edu.endDate === 'Present' ? 'Présent' : edu.endDate ? new Date(edu.endDate).getFullYear() : '?'}
                    </span>
                  </div>
